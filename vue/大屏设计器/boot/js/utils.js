...
function initData(callback) {
  ...
  if (id) {
    loadJs(dataPath+'/data' + id + '.js', callback);
  }
...
function getReportData(url,callback){
   axios.get(url).then(res=>{
      let report = res.data.result;
      if(!report){
        return;
      }
           report.desScreenItemJson = eval("("+report.desScreenItemJson+")");
           report.desScreenMainJson = eval("("+report.desScreenMainJson+")");
       callback(report);
   });
