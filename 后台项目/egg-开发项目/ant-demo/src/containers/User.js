...
import { Spin, Alert, List, Button,Select,Modal,Table,Icon, Divider,Popconfirm,Input} from 'antd';
import { requestTasks ,selectTasksMenu ,fetchTasks,removeTask,enqueueTask,
  fetchUsers,removeUser,receiveUsers,editUser
  } from '../actions'
...
info = (item) => {
  ...
  render() {
    let { userList, dispatch ,match} = this.props;
    let dataSource = userList || [];
    return(<div>
         <Link to={`${match.url}/new`}><Button style={{marginBottom:"4px"}}>新增用户</Button></Link>
